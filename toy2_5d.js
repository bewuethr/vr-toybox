...
function main() {
  ...
  function render(time) {
    ...
    if (resizeRendererToDisplaySize(renderer)) {
      ...
      const cameraZ =
        canvas.clientHeight / 2 / Math.tan((camera.fov / 2 / 180) * Math.PI) +
        3 * radius;
...
function createCamera(canvas, radius, fov = 45) {
  ...
  const near = 0.1;
  const cameraZ =
    canvas.clientHeight / 2 / Math.tan((fov / 2 / 180) * Math.PI) + 3 * radius;
...
function createBdMeshes(wallWidth, radius, canvas) {
  const vertBdGeometry = new THREE.BoxGeometry(
    2 * wallWidth,
    canvas.clientHeight - 2 * wallWidth,
    2 * radius,
  );
  const horzBdGeometry = new THREE.BoxGeometry(
    canvas.clientWidth - 2 * wallWidth,
    2 * wallWidth,
    2 * radius,
  );
  ...
  rightBdMesh.position.set(
    canvas.clientWidth - wallWidth,
    canvas.clientHeight / 2 + wallWidth,
    0,
  );
  bottomBdMesh.position.set(canvas.clientWidth / 2 + wallWidth, wallWidth, 0);
  topBdMesh.position.set(
    canvas.clientWidth / 2 - wallWidth,
    canvas.clientHeight - wallWidth,
    0,
  );
...
function updateBdMeshes(meshes, width, height, wallWidth, radius) {
  [meshes.left, meshes.right].forEach(
    (m) =>
  ...
  [meshes.top, meshes.bottom].forEach(
    (m) =>
      (m.geometry = new THREE.BoxGeometry(
        width - 2 * wallWidth,
        2 * wallWidth,
